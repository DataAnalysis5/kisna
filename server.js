...
mongoose
  .connect(
    "mongodb+srv://dikshantbuwa2_kisna_survey:KisnaSurvey12@kisna-survey.fwmmey6.mongodb.net/?appName=kisna-survey",
    {
    ...
    const formattedData = responses.map((response) => {
      ...
      if (response.surveyId?.questions) {
        response.surveyId.questions.forEach((question, index) => {
          ...
          // Handle different question types - dropdown is treated like radio/text
...
app.post("/api/surveys", async (req, res) => {
  try {
    ...
    // Validate questions - dropdown questions also need options to be removed if present
    surveyData.questions.forEach((question) => {
      if (question.type === "star") {
        delete question.options
      }
      // Dropdown questions KEEP their options - removed the dropdown case
    })
